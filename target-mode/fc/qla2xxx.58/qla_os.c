...
qla2x00_probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (qla_ini_mode_enabled(ha))
		...
		DEBUG(printk("skipping scsi_scan_host() for non-initiator port\n"));
...
qla2x00_do_dpc(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (!ha->flags.init_done || ha->flags.mbox_busy)
			goto end_loop;
		...
		if (test_bit(SCR_PENDING, &ha->dpc_flags)) {
			int ret;
			ret = qla2x00_send_change_request(ha, 0x3, 0);
			if (ret != QLA_SUCCESS)
				DEBUG(printk("Failed to enable receiving of RSCN "
				    "requests: 0x%x.\n", ret));
			clear_bit(SCR_PENDING, &ha->dpc_flags);
		}
	...
	case pci_channel_io_frozen:
		...
		if (IS_QLA82XX(ha)) {
			...
			DEBUG2(qla_printk(KERN_ERR, ha,
			    "scsi(%ld): Pci channel io frozen", ha->host_no));
