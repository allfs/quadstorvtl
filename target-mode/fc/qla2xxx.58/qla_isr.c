...
skip_rio:
	...
	case MBA_PORT_UPDATE:
		...
		if (ha->vp_idx == 0 && !qla_ini_mode_enabled(ha))
			set_bit(SCR_PENDING, &ha->dpc_flags);
...
qla2x00_status_entry(scsi_qla_host_t *ha, void *pkt)
{
	...
	case CS_DATA_UNDERRUN:
		...
		if (scsi_status & SS_RESIDUAL_UNDER) {
			...
			DEBUG2(qla_printk(KERN_INFO, ha,
			    "scsi(%ld:%d:%d) scsi_status: 0x%x, lscsi_status: "
			    "0x%x\n", ha->host_no, cp->device->id,
			    cp->device->lun, scsi_status, lscsi_status));
		}
...
qla24xx_process_response_queue(struct scsi_qla_host *ha)
{
	...
	while (ha->response_ring_ptr->signature != RESPONSE_PROCESSED) {
		...
		case NOTIFY_ACK_TYPE:
			qlt_response_pkt_all_vps(ha, (response_t *)pkt);
			break;
		case MARKER_TYPE:
			/* Do nothing in this case, this check is to prevent it
			 * from faling into default case
