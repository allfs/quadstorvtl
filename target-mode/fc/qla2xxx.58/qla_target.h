...
#define QLA_TGT_MAX_SG_24XX(ql) \
	(min_t(int, 1270, ((ql) > 0) ? (QLA_TGT_DATASEGS_PER_CMD_24XX + \
...
struct qla_tgt {
	...
	struct work_struct sess_del_work;
...
struct qla_cmd_hdr {
	int type;
	STAILQ_ENTRY(qla_cmd_hdr) q_list;
};

struct qla_tgt_cmd {
	struct qla_cmd_hdr cmd_h;
	...
	int local_pool;
	STAILQ_ENTRY(qla_tgt_cmd) q_list;
};
...
#define QLA24XX_MGMT_SEND_NACK	1
	...
	u32 unpacked_lun;
	uint32_t tag;
	STAILQ_ENTRY(qla_tgt_mgmt_cmd) q_list;
};
 ...
 */
extern int qlt_add_target(struct scsi_qla_host *);
extern int qlt_remove_target(struct scsi_qla_host *);
...
extern void qlt_vport_create(struct scsi_qla_host *);
...
extern void qlt_probe_one_stage1(struct scsi_qla_host *);
extern int qlt_mem_alloc(struct scsi_qla_host *);
extern void qlt_mem_free(struct scsi_qla_host *);
