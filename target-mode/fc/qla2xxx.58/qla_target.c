...
static void qlt_free_session_done(void *data)
{
	struct qla_tgt_sess *sess = data;
...
void qlt_unreg_sess(struct qla_tgt_sess *sess)
{
	...
	vha->tgt.tgt_ops->clear_nacl_from_fcport_map(sess);

	list_del(&sess->sess_list_entry);
	if (sess->deleted)
		list_del(&sess->del_list_entry);

	INIT_WORK(&sess->free_work, qlt_free_session_done, sess);
...
/* ha->hardware_lock supposed to be held on entry */
static inline void qlt_clear_tgt_db(struct qla_tgt *tgt, bool local_only)
{
	struct qla_tgt_sess *sess;
...
static void qlt_del_sess_work_fn(void *data)
{
	struct qla_tgt *tgt = data;
...
{
	struct scsi_qla_host *ha = tgt->ha;
	unsigned long flags;
	int res;
	/*
	 * We need to protect against race, when tgt is freed before or
	...
	uint16_t srr_flags, uint16_t srr_reject_code, uint8_t srr_explan)
{
	...
	pkt = (request_t *)qla2x00_alloc_iocbs(vha);
	if (!pkt) {
	...
	bool ids_reversed)
{
	...
	resp = (struct abts_resp_to_24xx *)qla2x00_alloc_iocbs(vha);
	if (!resp) {
	...
	struct abts_resp_from_24xx_fw *entry)
{
	...
	ctio = (struct ctio7_to_24xx *)qla2x00_alloc_iocbs(vha);
	if (ctio == NULL) {
	...
	struct qla_tgt_mgmt_cmd *mcmd, uint32_t resp_code)
{
	...
	ctio = (struct ctio7_to_24xx *)qla2x00_alloc_iocbs(ha);
	if (ctio == NULL) {
	...
	uint32_t req_cnt)
{
	...
	if (vha->req_q_cnt < (req_cnt + 2)) {
		cnt = (uint16_t)RD_REG_DWORD(&reg->isp24.req_q_out);

		ql_dbg(ql_dbg_tgt, vha, 0xe00a,
		    "Request ring circled: cnt=%d, vha->->ring_index=%d, "
		    "vha->req_q_cnt=%d, req_cnt=%d\n", cnt,
		    vha->req_ring_index, vha->req_q_cnt, req_cnt);
		if  (vha->req_ring_index < cnt)
			vha->req_q_cnt = cnt - vha->req_ring_index;
		else
			vha->req_q_cnt = vha->request_q_length -
			    (vha->req_ring_index - cnt);
	}

	if (unlikely(vha->req_q_cnt < (req_cnt + 2))) {
		ql_dbg(ql_dbg_tgt, vha, 0xe00b,
		    "qla_target(%d): There is no room in the "
		    "request ring: vha->req_ring_index=%d, vha->req_q_cnt=%d, "
		    "req_cnt=%d\n", vha->vp_idx, vha->req_ring_index,
		    vha->req_q_cnt, req_cnt);
		return -EAGAIN;
	}
	vha->req_q_cnt -= req_cnt;
...
static inline void *qlt_get_req_pkt(struct scsi_qla_host *vha)
{
	/* Adjust ring index. */
	vha->req_ring_index++;
	if (vha->req_ring_index == vha->request_q_length) {
		vha->req_ring_index = 0;
		vha->request_ring_ptr = vha->request_ring;
	} else {
		vha->request_ring_ptr++;
	}
	return (cont_entry_t *)vha->request_ring_ptr;
}
	...
	struct scsi_qla_host *vha)
{
	...
	pkt = (struct ctio7_to_24xx *)vha->request_ring_ptr;
	prm->pkt = pkt;
...
static inline int qlt_need_explicit_conf(struct scsi_qla_host *ha,
 ...
 */
static void qlt_do_work(void *data)
{
	struct qla_tgt_cmd *cmd = data;
	...
	ret = vha->tgt.tgt_ops->handle_cmd(vha, cmd, cdb, data_length,
	...
	struct atio_from_isp *atio)
{
	...
	INIT_WORK(&cmd->work, qlt_do_work, cmd);
...
{
	struct scatterlist *sg, *sgp, *sg_srr;
	size_t first_offset = 0, rem_offset = offset, tmp = 0;
	int i, sg_srr_cnt, bufflen = 0, sg_srr_start = -1;
	...
	for (i = 0; i < cmd->sg_cnt; i++) {
		ql_dbg(ql_dbg_tgt, cmd->vha, 0xe024,
		    ...
		    i, sg, sg->page, sg->length, sg->offset);
	 ...
	 */
	for (i = sg_srr_start; i < cmd->sg_cnt; i++) {
		sg = &cmd->sg[i];
		if (first_offset) {
			sgp->page = sg->page;
			sgp->offset = first_offset;
			sgp->length = (sg->length - first_offset); 
			first_offset = 0;
		} else {
			sgp->page = sg->page;
			sgp->length = sg->length;
			sgp->offset = 0;
		}
		bufflen += sgp->length;
		sgp++;
...
static void qlt_handle_srr_work(void *data)
{
	struct qla_tgt *tgt = data;
	struct scsi_qla_host *vha = tgt->vha;
...
static void qlt_sess_work_fn(void *data)
{
	struct qla_tgt *tgt = data;
	struct scsi_qla_host *vha = tgt->vha;
	unsigned long flags;
...
int qlt_add_target(struct scsi_qla_host *base_vha)
{
	struct scsi_qla_host *ha = base_vha;
	struct qla_tgt *tgt;
	...
	tgt->fcbridge = fcbridge_new(base_vha, base_vha->host_no);
	if (!tgt->fcbridge) {
		kfree(tgt);
		return -ENOMEM;
	}
	...
	INIT_WORK(&tgt->sess_del_work, qlt_del_sess_work_fn, tgt);
	spin_lock_init(&tgt->sess_work_lock);
	INIT_WORK(&tgt->sess_work, qlt_sess_work_fn, tgt);
	INIT_LIST_HEAD(&tgt->sess_works_list);
	spin_lock_init(&tgt->srr_lock);
	INIT_LIST_HEAD(&tgt->srr_ctio_list);
	INIT_LIST_HEAD(&tgt->srr_imm_list);
	INIT_WORK(&tgt->srr_work, qlt_handle_srr_work, tgt);
	atomic_set(&tgt->tgt_global_resets_count, 0);

	ha->tgt.qla_tgt = tgt;
	ha->tgt.tgt_ops = &qla_sc_template;
	...
	/* 3 is reserved */
	tgt->sg_tablesize = QLA_TGT_MAX_SG_24XX(base_vha->request_q_length - 3);
...
/* Must be called under tgt_host_action_mutex */
int qlt_remove_target(struct scsi_qla_host *vha)
{
	struct scsi_qla_host *ha = vha;
 ...
 * the target mode specific struct scsi_qla_host and struct scsi_qla_host
 * members.
 */
void
qlt_vport_create(struct scsi_qla_host *vha)
{
	struct scsi_qla_host *ha = vha;
...
qlt_24xx_config_nvram_stage1(struct scsi_qla_host *vha, struct nvram_24xx *nv)
{
	...
	if (qla_tgt_mode_enabled(vha)) {
		...
		nv->exchange_count = __constant_cpu_to_le16(0x400);
...
void
qlt_probe_one_stage1(struct scsi_qla_host *ha)
{
	if (!QLA_TGT_MODE_ENABLED())
		return;
...
int
qlt_mem_alloc(struct scsi_qla_host *ha)
{
	if (!QLA_TGT_MODE_ENABLED())
		return 0;
...
void
qlt_mem_free(struct scsi_qla_host *ha)
{
	if (!QLA_TGT_MODE_ENABLED())
		return;
...
qlt_update_vp_map(struct scsi_qla_host *vha, int cmd)
{
	...
	switch (cmd) {
	case SET_VP_IDX:
		vha->tgt.tgt_vp_map[vha->vp_idx].vha = vha;
		break;
	case SET_AL_PA:
		vha->tgt.tgt_vp_map[vha->d_id.b.al_pa].idx = vha->vp_idx;
		break;
	case RESET_VP_IDX:
		vha->tgt.tgt_vp_map[vha->vp_idx].vha = NULL;
		break;
	case RESET_AL_PA:
		vha->tgt.tgt_vp_map[vha->d_id.b.al_pa].idx = 0;
		break;
	}
}
...
int __init qlt_init(void)
{
	...
	qla_tgt_wq = create_workqueue("qla_tgt_wq");
	if (!qla_tgt_wq) {
