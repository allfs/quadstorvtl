 ...
 * ha:   Pointer to the scsi_qla_host_t.
 * id:    This is a unique identifier for the level. It identifies the
 *        part of the code from where the message originated.
 * msg:   The message to be displayed.
 */
void
ql_dbg(uint32_t level, scsi_qla_host_t *ha, int32_t id, const char *fmt, ...)
{
	va_list va;
	...
	if (ha != NULL) {
		const struct pci_dev *pdev = ha->pdev;
 ...
 * It is to be used when ha is available. It formats the message
 ...
 */
void
ql_log(uint32_t level, scsi_qla_host_t *ha, int32_t id, const char *fmt, ...)
{
	va_list va;
	char pbuf[128];
	...
	if (ha != NULL) {
		...
		snprintf(pbuf, sizeof(pbuf), "%s [%s]-%04x:%ld: ",
			QL_MSGHDR, dev_name(&(pdev->dev)), id, ha->host_no);
	} else {
