...
qla2x00_mailbox_command(scsi_qla_host_t *pvha, mbx_cmd_t *mcp)
{
	...
	if (ha->flags.isp82xx_fw_hung) {
		...
		DEBUG(qla_printk(KERN_ERR, ha,
		    "%s(%ld): Firmware in hung state FW hung = %d\n",
		    __func__, ha->host_no, ha->flags.isp82xx_fw_hung));
		return QLA_FUNCTION_TIMEOUT;
	}
	...
	if (!wait_for_completion_timeout(&ha->mbx_cmd_comp,
		...
		DEBUG2_3_11(printk("%s(%ld): cmd access timeout, cmd=0x%x, "
		...
		if (!io_lock_on || (mcp->flags & IOCTL_CMD) ||
			...
			if (!test_bit(ISP_ABORT_NEEDED, &ha->dpc_flags) &&
				...
				qla_printk(KERN_WARNING, ha,
				    ...
				    "cmd=0x%x, mb[0]=0x%x, eeh_busy=0x%x. "
				    "Scheduling ISP abort.\n", command,
				    mcp->mb[0], ha->flags.eeh_busy);
			...
			if (!test_bit(ISP_ABORT_NEEDED, &ha->dpc_flags) &&
				...
				qla_printk(KERN_WARNING, ha,
				    "Mailbox command timeout occured, cmd=0x%x "
				    "mb[0]=0x%x. Issuing ISP abort.\n",
				    command, mcp->mb[0]);
 ...
 */
int
qla2x00_get_node_name_list(scsi_qla_host_t *ha, void **out_data, int *out_len)
{
	...
	while (left > 0) {
		...
		pmap = dma_alloc_coherent(&ha->pdev->dev, dma_size,
					 &pmap_dma, GFP_KERNEL);
		if (!pmap) {
			DEBUG2_3(printk("%s(%ld): DMA Alloc failed of %ld\n",
		...
		if (!list) {
			DEBUG2_3(printk("%s(%ld): failed to allocate node names list "
			    "structure.\n", __func__, ha->host_no));
			rval = QLA_MEMORY_ALLOC_FAILED;
			goto out_free;
		}
    ...
    uint8_t area, uint8_t al_pa, uint16_t *mb, uint8_t opt)
{
	...
	rval = qla2x00_issue_iocb_timeout(ha, lg, lg_dma, 0,
	    (ha->r_a_tov / 10 * 2) + 2);
	if (rval != QLA_SUCCESS) {
...
int
qla2x00_read_sfp(scsi_qla_host_t *ha, dma_addr_t sfp_dma, uint8_t *sfp,
...
int
qla2x00_write_sfp(scsi_qla_host_t *ha, dma_addr_t sfp_dma, uint8_t *sfp,
	uint16_t dev, uint16_t off, uint16_t len, uint16_t opt)
{
	int rval;
	mbx_cmd_t mc;
...
int
qla81xx_write_mpi_register(scsi_qla_host_t *ha, uint16_t *mb)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	mcp->mb[0] = MBX_WRITE_MPI_REGISTER;
	mcp->mb[1] = mb[0];
	mcp->mb[2] = mb[1];
	mcp->mb[3] = mb[2];
	mcp->mb[4] = mb[3];
...
qla2x00_get_thermal_temp(scsi_qla_host_t *ha, uint16_t *temp, uint16_t *frac)
{
	...
	/* Integer part */
	rval = qla2x00_read_sfp(ha, 0, &byte, 0x98, 0x01, 1, BIT_13|BIT_0);
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(printk(KERN_WARNING
			"%s(%ld): failed=%x.\n", __func__, ha->host_no, rval));
	...
	*temp = byte;
	...
	/* Fraction part */
	rval = qla2x00_read_sfp(ha, 0, &byte, 0x98, 0x10, 1, BIT_13|BIT_0);
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(printk(KERN_WARNING
			"%s(%ld): failed=%x.\n", __func__, ha->host_no, rval));
		ha->flags.thermal_supported = 0;
...
int
qla82xx_mbx_beacon_ctl(scsi_qla_host_t *ha, int enable)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
...
int
qla82xx_md_get_template_size(scsi_qla_host_t *ha)
{
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(qla_printk(KERN_WARNING, ha,
		    "%s(%ld): mailbox command FAILED=0x%x, subcode=%x.\n",
		    __func__, ha->host_no,
		...
		if (!ha->md_template_size) {
			DEBUG11(qla_printk(KERN_WARNING, ha,
			    "%s(%ld): Null template size obtained.\n",
			    __func__, ha->host_no));
			rval = QLA_FUNCTION_FAILED;
		}
	}
...
int
qla82xx_md_get_template(scsi_qla_host_t *ha)
{
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	int rval = QLA_FUNCTION_FAILED;
...
int
qla2x00_port_logout(scsi_qla_host_t *ha, struct fc_port *fcport)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	if (IS_QLA2100(ha) || IS_QLA2200(ha)) {
		DEBUG11(printk("%s(%ld): Implicit LOGO Unsupported\n", __func__,
		    ha->host_no));
		return QLA_FUNCTION_FAILED;
	}
