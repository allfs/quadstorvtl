 ...
 */
int
qla81xx_reset_mpi(scsi_qla_host_t *ha)
{
	uint16_t mb[4] = { 0x1010, 0, 1, 0 };

	return qla81xx_write_mpi_register(ha, mb);
}
...
qla24xx_reset_risc(scsi_qla_host_t *ha)
{
	...
	if (test_and_clear_bit(MPI_RESET_NEEDED, &ha->dpc_flags)) {
		spin_unlock_irqrestore(&ha->hardware_lock, flags);
		if (qla81xx_reset_mpi(ha) != QLA_SUCCESS) {
			set_bit(MPI_RESET_NEEDED, &ha->dpc_flags);
...
enable_82xx_npiv:
			...
			if (rval == QLA_SUCCESS) {
				...
				if (fw_major_version == 0) {
					...
					if (ql2xallocfwdump && !IS_QLA82XX(ha))
...
{
	struct device_reg_24xx __iomem *reg = &ha->iobase->isp24;
	device_reg_t __iomem *regbase = ha->iobase;
	...
	qlt_24xx_config_rings(ha, regbase);
...
qla2x00_configure_hba(scsi_qla_host_t *ha)
{
	...
	down(&ha->vport_sem);
	qlt_update_vp_map(ha, SET_AL_PA);
	up(&ha->vport_sem);
...
qla2x00_rport_del(void *data)
{
	...
	spin_lock_irqsave(fcport->ha->host->host_lock, flags);
	rport = fcport->drport;
	fcport->drport = NULL;
	spin_unlock_irqrestore(fcport->ha->host->host_lock, flags);
	if (rport) {
		 ...
		 */
		qlt_fc_port_deleted(fcport->ha, fcport);
	}
}
...
qla24xx_update_fcport_fcp_prio(scsi_qla_host_t *ha, fc_port_t *fcport)
{
	...
	if (ret == QLA_SUCCESS) {
		if (fcport->fcp_prio != priority)
			DEBUG2(printk(KERN_INFO"Updated FCP_CMND priority - "
			    "value=%d loop_id=%d port_id=%02x%02x%02x.\n",
			    priority, fcport->loop_id, fcport->d_id.b.domain,
			    fcport->d_id.b.area, fcport->d_id.b.al_pa));
		fcport->fcp_prio = priority;
	} else
		DEBUG2(printk(
		    "scsi(%ld): Unable to update FCP_CMND priority - ret=0x%x for "
...
qla2x00_configure_fabric(scsi_qla_host_t *ha)
{
	...
	do {
		...
		rval = ha->isp_ops->fabric_login(ha, loop_id, 0xff, 0xff,
		    0xfc, mb, BIT_1 | BIT_0);
		if (rval != QLA_SUCCESS) {
			set_bit(LOOP_RESYNC_NEEDED, &ha->dpc_flags);
			return rval;
    ...
    uint16_t *next_loopid)
{
	...
	for (;;) {
		...
		rval = ha->isp_ops->fabric_login(ha, fcport->loop_id,
		    fcport->d_id.b.domain, fcport->d_id.b.area,
	...
	if (!status && !(status = qla2x00_init_rings(ha))) {
		...
		if (!(status = qla2x00_fw_ready(ha))) {
			...
			if (qla_tgt_mode_enabled(ha))
				qlt_24xx_process_atio_queue(ha);
			spin_unlock_irqrestore(&ha->hardware_lock, flags);
...
qla24xx_nvram_config(scsi_qla_host_t *ha)
{
	...
	qlt_24xx_config_nvram_stage1(ha, nv);
	...
	qlt_24xx_config_nvram_stage2(ha, icb);
...
qla24xx_configure_vhba(scsi_qla_host_t *ha)
{
	...
	/* Login to SNS first */
	rval2 = qla24xx_login_fabric(ha, NPH_SNS, 0xff, 0xff, 0xfc,
	    mb, BIT_1);
	if (rval2 != QLA_SUCCESS || mb[0] != MBS_COMMAND_COMPLETE) {
		if (rval2 == QLA_MEMORY_ALLOC_FAILED)
			DEBUG15(qla_printk(KERN_INFO, ha,
			    "Failed SNS login: loop_id=0x%x, rval2=%d\n",
			    NPH_SNS, rval2));
		else
			DEBUG15(qla_printk(KERN_INFO, ha, "Failed SNS login: "
			    "loop_id=%x mb[0]=%x mb[1]=%x mb[2]=%x mb[6]=%x "
			    "mb[7]=%x\n", NPH_SNS, mb[0], mb[1], mb[2],
			    mb[6], mb[7]));
		return (QLA_FUNCTION_FAILED);
	}
...
qla82xx_restart_isp(scsi_qla_host_t *ha)
{
	...
	if (!status) {
		...
		/* Update the FW versions */
		status = qla82xx_check_md_needed(ha);
