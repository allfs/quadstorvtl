...
qla2x00_mgmt_svr_login(scsi_qla_host_t *ha)
{
	...
	rval = ha->isp_ops->fabric_login(ha, ha->mgmt_svr_loop_id, 0xff, 0xff,
	    0xfa, mb, BIT_1 | BIT_0);
	if (rval != QLA_SUCCESS || mb[0] != MBS_COMMAND_COMPLETE) {
		if (rval == QLA_MEMORY_ALLOC_FAILED)
			DEBUG2_13(printk("%s(%ld): Failed MANAGEMENT_SERVER "
			    "login: loopid=0x%x rval=%d\n", __func__,
			    ha->host_no, ha->mgmt_svr_loop_id, rval));
		else
			DEBUG2_13(printk("%s(%ld): Failed MANAGEMENT_SERVER "
			    "login: loop_id=%x mb[0]=%x mb[1]=%x mb[2]=%x "
			    "mb[6]=%x mb[7]=%x\n", __func__, ha->host_no,
			    ha->mgmt_svr_loop_id, mb[0], mb[1], mb[2], mb[6],
			    mb[7]));
