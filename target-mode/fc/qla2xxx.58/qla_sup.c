...
qla24xx_get_flash_version(scsi_qla_host_t *ha, void *mbuf)
{
	...
	dcode = mbuf;
	ha->isp_ops->read_optrom(ha, (uint8_t *)dcode,
	    ha->flt_region_gold_fw << 2, 32);

	if (dcode[4] == 0xFFFFFFF && dcode[5] == 0xFFFFFFFF &&
	    dcode[6] == 0xFFFFFFF && dcode[7] == 0XFFFFFFFF) {
		DEBUG2(qla_printk(KERN_INFO, ha,
		    "%s(%ld): Unrecognized golder fw at 0x%x.\n",
		    __func__, ha->host_no, ha->flt_region_gold_fw * 4));
		return ret;
	}
