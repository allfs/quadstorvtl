...
#ifdef LINUX
int
check_multipath(struct physdevice *device, int israid)
{
	char cmd[256];
	int retval;
	char devnumbers[30];
	...
	retval = get_dev_numbers(device, devnumbers, israid);
	if (retval != 0)
	{
		return -1;
	}

	sprintf(cmd, "%s deps | grep %s", DMSETUP_PROG, devnumbers);
	fp = popen(cmd, "r");
	if (!fp)
	{
		return -1;
	...
	*tmp = 0;
	...
	sprintf(cmd, "%s ls --target=multipath | grep \"%s\"", DMSETUP_PROG, buf);
	fp = popen(cmd, "r");
	if (!fp)
	{
		return -1;
	...
	while (fgets(tmp1, sizeof(tmp1), fp))
	{
	...
	device->multipath = 1;
	sprintf(device->mdevname, "/dev/mapper/%s", buf);
	return 0;
}
