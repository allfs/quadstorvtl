...
static void vqtl_init_inquiry_data(struct mchanger* mchanger)
{
...
static int
vqtl_device_identification(struct mchanger *mchanger, uint8_t *buffer, int length)
{
	return mchanger_device_identification(mchanger, buffer, length);
}

static int
vqtl_serial_number(struct mchanger *mchanger, uint8_t *buffer, int length)
{
	return mchanger_serial_number(mchanger, buffer, length);
}

static int
vqtl_evpd_inquiry(struct mchanger *mchanger, struct qsio_scsiio *ctio, uint8_t page_code, uint16_t allocation_length)
{
	int retval;
	...
	switch (page_code)
	{
		case UNIT_SERIAL_NUMBER_PAGE:
			retval = vqtl_serial_number(mchanger, ctio->data_ptr, allocation_length);
			if (retval < 0)
				goto err;
			ctio->dxfer_len = retval;
			break;
		case DEVICE_IDENTIFICATION_PAGE:
			retval = vqtl_device_identification(mchanger, ctio->data_ptr, allocation_length);
			if (retval < 0)
				goto err;
			ctio->dxfer_len = retval;
			break;
...
void
vqtl_init_handlers(struct mchanger *mchanger)
{
	struct mchanger_handlers *handlers = &mchanger->handlers;
	char *vendor_id, *product_id;

	handlers->init_inquiry_data = vqtl_init_inquiry_data;
	handlers->evpd_inquiry = vqtl_evpd_inquiry;
	...
	switch (mchanger->make)
	{
		case LIBRARY_TYPE_VQUANTUM_M2500:
			vendor_id = VENDOR_ID_ATL;
			product_id = PRODUCT_ID_VQUANTUM_M2500;
			break;
		default:
			debug_check(1);
			return;
