...
mchanger_copy_vital_product_page_info(struct mchanger *mchanger, uint8_t *buffer, uint16_t allocation_length)
{
	...
	for (i = 0; i < mchanger->evpd_info.num_pages; i++) {
		page->page_type[i] = mchanger->evpd_info.page_code[i];
	}

	return min_t(int, allocation_length, mchanger->evpd_info.num_pages + sizeof(*page));
}	
...
int
mchanger_device_identification(struct mchanger *mchanger, uint8_t *buffer, int allocation_length)
{
	...
	memcpy(identifier, &mchanger->unit_identifier, idlength);
	min_len = min_t(int, allocation_length, idlength + sizeof(*page));
	return min_len;
}

int
mchanger_serial_number(struct mchanger *mchanger, uint8_t *buffer, int allocation_length)
{
	...
	memcpy(page->serial_number, mchanger->unit_identifier.serial_number, mchanger->serial_len);

	min_len = min_t(int, allocation_length, mchanger->serial_len + sizeof(*page));
	return min_len;
}
